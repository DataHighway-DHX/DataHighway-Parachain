...
fn setup_collator_candidates<T: Config>(
    ...
    for acc in collators.iter() {
        ...
        assert_ok!(<Pallet<T>>::join_candidates(T::Origin::from(Some(acc.clone()).into()), amount,));
        assert_eq!(<CandidatePool<T>>::get(acc).unwrap().stake, amount);
    }

    TopCandidates::<T>::get().into_bounded_vec().into_inner().drain(..).map(|c| c.owner).collect()
}
