...
fn should_select_collators_genesis_session() {
    ExtBuilder::default()
        ...
        .build()
        .execute_with(|| {
            assert_eq!(StakePallet::new_session(0).expect("first session must return new collators").len(), 2);
            assert_eq!(StakePallet::new_session(1).expect("second session must return new collators").len(), 2);
        });
}
...
fn genesis() {
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            // Safety first checks
            assert_eq!(StakePallet::max_selected_candidates(), <Test as Config>::MinCollators::get());
            assert_eq!(
                StakePallet::round(),
    ...
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            // Safety first checks
            assert_eq!(StakePallet::max_selected_candidates(), <Test as Config>::MinCollators::get());
            assert_eq!(StakePallet::round(), RoundInfo::new(0, 0, <Test as Config>::DefaultBlocksPerRound::get()));
        });
}
...
fn join_collator_candidates() {
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            assert_noop!(StakePallet::join_candidates(Origin::signed(1), 11u128,), Error::<Test>::CandidateExists);
            assert_noop!(StakePallet::join_delegators(Origin::signed(1), 1, 11u128,), Error::<Test>::CandidateExists);
            assert_noop!(StakePallet::join_candidates(Origin::signed(3), 11u128,), Error::<Test>::DelegatorExists);
            assert_noop!(StakePallet::join_candidates(Origin::signed(7), 9u128,), Error::<Test>::ValStakeBelowMin);
            assert_noop!(
            ...
            assert_eq!(last_event(), MetaEvent::StakePallet(Event::JoinedCollatorCandidates(7, 10u128)));
            ...
            assert_ok!(StakePallet::join_candidates(Origin::signed(10), StakePallet::max_candidate_stake()));
...
fn collator_exit_executes_after_delay() {
    ExtBuilder::default()
        .with_balances(vec![(1, 1000), (2, 300), (3, 110), (4, 100), (5, 100), (6, 100), (7, 100), (8, 9), (9, 4)])
        ...
        .execute_with(|| {
            ...
            roll_to(4, vec![]);
            assert_noop!(StakePallet::init_leave_candidates(Origin::signed(3)), Error::<Test>::CandidateNotFound);
...
fn collator_selection_chooses_top_candidates() {
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            assert_eq!(last_event(), MetaEvent::StakePallet(Event::JoinedCollatorCandidates(6, 69u128)));
...
fn exit_queue_with_events() {
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            assert_noop!(StakePallet::init_leave_candidates(Origin::signed(4)), Error::<Test>::AlreadyLeaving);
    ...
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            for owner in vec![1, 2, 5, 6, 7, 8, 9, 10].iter() {
                assert!(
                    StakePallet::candidate_pool(owner).unwrap().can_exit(1 + <Test as Config>::ExitQueueDelay::get())
                );
            }
            ...
            for collator in vec![1u64, 2u64, 5u64, 6u64, 7u64].iter() {
                assert_ok!(StakePallet::execute_leave_candidates(Origin::signed(*collator), *collator));
...
fn multiple_delegations() {
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            assert!(!StakePallet::candidate_pool(2).unwrap().delegators.contains(&StakeOf::<Test> {
                owner: 9,
                amount: 10
            }));
            ...
            roll_to(31, vec![]);
            let mut new3 =
                vec![Event::LeftTopCandidates(2), Event::CollatorScheduledExit(5, 2, 7), Event::NewRound(30, 6)];
            expected.append(&mut new3);
            assert_eq!(events(), expected);

            // test join_delegator errors
            assert_ok!(StakePallet::delegate_another_candidate(Origin::signed(8), 1, 10));
            assert_noop!(StakePallet::join_delegators(Origin::signed(12), 1, 10), Error::<Test>::TooManyDelegators);
            assert_noop!(
            ...
            assert_noop!(StakePallet::unlock_unstaked(Origin::signed(12), 12), Error::<Test>::UnstakingIsEmpty);
...
fn should_update_total_stake() {
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            assert_noop!(StakePallet::delegator_stake_more(Origin::signed(7), 1, 0), Error::<Test>::ValStakeZero);
            assert_noop!(StakePallet::delegator_stake_less(Origin::signed(7), 1, 0), Error::<Test>::ValStakeZero);
            assert_eq!(
                StakePallet::total_collator_stake(),
                TotalStake {
            ...
            assert_eq!(StakePallet::candidate_pool(2).unwrap().stake, StakePallet::candidate_pool(3).unwrap().stake);
            assert_ok!(StakePallet::init_leave_candidates(Origin::signed(2)));
...
fn author_notarization() {
    ExtBuilder::default()
        .with_balances(vec![(1, 50), (2, 50), (3, 50), (4, 50), (5, 50)])
        .with_collators(vec![(1, 10), (2, 20)])
        .with_delegators(vec![(3, 1, 10), (4, 1, 10), (5, 2, 30)])
        .build()
        .execute_with(|| {
            ...
            <crate::RewardPerBlock<Test>>::put(20);
            roll_to(2, vec![None, Some(1)]);

            assert_eq!(Balances::usable_balance(&1), collator_one + 7);
            assert_eq!(Balances::usable_balance(&2), collator_two);
            assert_eq!(Balances::usable_balance(&3), delegator_three + 7);
            assert_eq!(Balances::usable_balance(&4), delegator_four + 7);
...
fn delegators_bond() {
    ExtBuilder::default()
        ...
        .execute_with(|| {
            roll_to(4, vec![]);
            assert_noop!(StakePallet::join_delegators(Origin::signed(6), 2, 50), Error::<Test>::AlreadyDelegating);
            assert_noop!(StakePallet::delegator_stake_more(Origin::signed(1), 2, 50), Error::<Test>::DelegatorNotFound);
            assert_noop!(StakePallet::delegator_stake_less(Origin::signed(1), 2, 50), Error::<Test>::DelegatorNotFound);
            assert_noop!(
                StakePallet::delegator_stake_more(Origin::signed(6), 2, 50),
                Error::<Test>::DelegationNotFound
            );
            assert_noop!(StakePallet::delegator_stake_more(Origin::signed(7), 6, 50), Error::<Test>::CandidateNotFound);
            assert_noop!(StakePallet::delegator_stake_less(Origin::signed(7), 6, 50), Error::<Test>::CandidateNotFound);
            assert_noop!(StakePallet::delegator_stake_less(Origin::signed(6), 1, 11), Error::<Test>::Underflow);
            assert_noop!(StakePallet::delegator_stake_less(Origin::signed(6), 1, 8), Error::<Test>::DelegationBelowMin);
            assert_noop!(StakePallet::delegator_stake_less(Origin::signed(6), 1, 6), Error::<Test>::NomStakeBelowMin);
            assert_ok!(StakePallet::delegator_stake_more(Origin::signed(6), 1, 10));
            assert_noop!(StakePallet::delegator_stake_less(Origin::signed(6), 2, 5), Error::<Test>::DelegationNotFound);
            assert_noop!(
                StakePallet::delegator_stake_more(Origin::signed(6), 1, 81),
                BalancesError::<Test>::InsufficientBalance
            );
            assert_noop!(StakePallet::join_delegators(Origin::signed(10), 1, 4), Error::<Test>::NomStakeBelowMin);

            roll_to(9, vec![]);
            assert_eq!(Balances::usable_balance(&6), 80);
            assert_ok!(StakePallet::init_leave_candidates(Origin::signed(1)));
            assert!(StakePallet::candidate_pool(1).unwrap().can_exit(1 + <Test as Config>::ExitQueueDelay::get()));
...
fn revoke_delegation_or_leave_delegators() {
    ExtBuilder::default()
        ...
        .execute_with(|| {
            roll_to(4, vec![]);
            assert_noop!(StakePallet::revoke_delegation(Origin::signed(1), 2), Error::<Test>::DelegatorNotFound);
            assert_noop!(StakePallet::revoke_delegation(Origin::signed(6), 2), Error::<Test>::DelegationNotFound);
            assert_noop!(StakePallet::leave_delegators(Origin::signed(1)), Error::<Test>::DelegatorNotFound);
            ...
            // MinDelegatorStake
            assert_noop!(StakePallet::revoke_delegation(Origin::signed(6), 2), Error::<Test>::NomStakeBelowMin);
            assert_noop!(StakePallet::revoke_delegation(Origin::signed(6), 3), Error::<Test>::NomStakeBelowMin);
...
fn round_transitions() {
    ...
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            assert_noop!(StakePallet::set_blocks_per_round(Origin::root(), 1), Error::<Test>::CannotSetBelowMin);
...
fn coinbase_rewards_few_blocks_detailed_check() {
    const DAY: u64 = <Test as Config>::BLOCKS_PER_YEAR / 365;
    ExtBuilder::default()
        .with_balances(vec![
        ...
        .execute_with(|| {
            ...
            let reward_per_block = 500 * DECIMALS / DAY as u128;
            <crate::RewardPerBlock<Test>>::put(reward_per_block);
            ...
            let reward_1 = 9915126512000;
            let reward_2 = 23135295200000;
            let reward_3 = 39660506048000;
            let reward_4 = 19830253024000;
            let reward_5 = 46270590400000;
            ...
            assert_eq!(Balances::usable_balance(&1), user_1 + reward_1);
            assert_eq!(Balances::usable_balance(&2), user_2);
            assert_eq!(Balances::usable_balance(&3), user_3 + reward_3);
            assert_eq!(Balances::usable_balance(&4), user_4 + reward_4);
            assert_eq!(Balances::usable_balance(&5), user_5);

            // 1 is block author for 2nd block
            roll_to(3, authors.clone());
            assert_eq!(Balances::usable_balance(&1), user_1 + reward_1 * 2);
            assert_eq!(Balances::usable_balance(&2), user_2);
            assert_eq!(Balances::usable_balance(&3), user_3 + reward_3 * 2);
            assert_eq!(Balances::usable_balance(&4), user_4 + reward_4 * 2);
            assert_eq!(Balances::usable_balance(&5), user_5);

            // 1 is block author for 3rd block
            roll_to(4, authors.clone());
            assert_eq!(Balances::usable_balance(&1), user_1 + reward_1 * 3);
            assert_eq!(Balances::usable_balance(&2), user_2);
            assert_eq!(Balances::usable_balance(&3), user_3 + reward_3 * 3);
            assert_eq!(Balances::usable_balance(&4), user_4 + reward_4 * 3);
            assert_eq!(Balances::usable_balance(&5), user_5);

            // 2 is block author for 4th block
            roll_to(5, authors.clone());
            assert_eq!(Balances::usable_balance(&1), user_1 + reward_1 * 3);
            assert_eq!(Balances::usable_balance(&2), user_2 + reward_2);
            assert_eq!(Balances::usable_balance(&3), user_3 + reward_3 * 3);
            assert_eq!(Balances::usable_balance(&4), user_4 + reward_4 * 3);
            assert_eq!(Balances::usable_balance(&5), user_5 + reward_5);
            assert_ok!(StakePallet::revoke_delegation(Origin::signed(5), 2));

            // since it is removed from delegation
            let new_reward_5 = 0;
            // since collator is only left
            let new_reward_2 = 69405885616000; // not exact as reward_per_block. Some precision lost

            // 2 is block author for 5th block
            roll_to(6, authors);
            assert_eq!(Balances::usable_balance(&1), user_1 + reward_1 * 3);
            assert_eq!(Balances::usable_balance(&2), user_2 + reward_2 + new_reward_2);
            assert_eq!(Balances::usable_balance(&3), user_3 + reward_3 * 3);
            assert_eq!(Balances::usable_balance(&4), user_4 + reward_4 * 3);
            // should not receive rewards due to revoked delegation
            assert_eq!(Balances::usable_balance(&5), user_5 + reward_5 + new_reward_5);
        });
}
...
fn delegator_should_not_receive_rewards_after_revoking() {
    ...
    ExtBuilder::default()
        ...
        .build()
        .execute_with(|| {
            <crate::RewardPerBlock<Test>>::put(3_000_000 * DECIMALS);
    ...
    ExtBuilder::default()
        .with_balances(vec![(1, 10_000_000 * DECIMALS), (2, 10_000_000 * DECIMALS), (3, 10_000_000 * DECIMALS)])
        .with_collators(vec![(1, 10_000_000 * DECIMALS)])
...
fn reach_max_top_candidates() {
    ExtBuilder::default()
        ...
        .with_collators(vec![(1, 10), (2, 20), (3, 10), (4, 10), (5, 10), (6, 10), (7, 10), (8, 10), (9, 10), (10, 10)])
        .build()
        .execute_with(|| {
            assert_eq!(
...
fn should_estimate_current_session_progress() {
    ExtBuilder::default()
        ...
        .build()
        .execute_with(|| {
            assert_eq!(StakePallet::estimate_current_session_progress(10).0.unwrap(), Permill::from_percent(10));
            assert_eq!(StakePallet::estimate_current_session_progress(20).0.unwrap(), Permill::from_percent(20));
            assert_eq!(StakePallet::estimate_current_session_progress(30).0.unwrap(), Permill::from_percent(30));
            assert_eq!(StakePallet::estimate_current_session_progress(60).0.unwrap(), Permill::from_percent(60));
            assert_eq!(StakePallet::estimate_current_session_progress(100).0.unwrap(), Permill::from_percent(100));
        });
}
...
fn set_max_selected_candidates_safe_guards() {
    ExtBuilder::default().with_balances(vec![(1, 10)]).with_collators(vec![(1, 10)]).build().execute_with(|| {
        assert_noop!(
        ...
        assert_ok!(StakePallet::set_max_selected_candidates(Origin::root(), <Test as Config>::MinCollators::get() + 1));
    });
}
...
fn set_max_selected_candidates_total_stake() {
    ...
    ExtBuilder::default()
        ...
        .with_collators(vec![(1, 11), (2, 12), (3, 13), (4, 14), (5, 15), (6, 16), (7, 17), (8, 18)])
        .with_delegators(vec![
            (11, 1, 21),
            (12, 2, 22),
            (13, 3, 23),
...
fn unlock_unstaked() {
    ...
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            roll_to(7, vec![]);
            assert_noop!(StakePallet::candidate_stake_less(Origin::signed(1), 10), Error::<Test>::NoMoreUnstaking);
            assert_noop!(StakePallet::delegator_stake_less(Origin::signed(2), 1, 10), Error::<Test>::NoMoreUnstaking);
            assert_ok!(StakePallet::unlock_unstaked(Origin::signed(1), 1));
            assert_ok!(StakePallet::unlock_unstaked(Origin::signed(2), 2));
            unstaking.remove(&4);
            unstaking.remove(&5);
...
fn kick_candidate_with_full_unstaking() {
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            assert_noop!(StakePallet::candidate_stake_less(Origin::signed(3), 1), Error::<Test>::NoMoreUnstaking);
...
fn kick_delegator_with_full_unstaking() {
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            // Additional unstake should fail
            System::set_block_number(100);
            assert_noop!(StakePallet::delegator_stake_less(Origin::signed(5), 1, 1), Error::<Test>::NoMoreUnstaking);
...
fn candidate_leaves() {
    ...
    ExtBuilder::default()
        ...
        .build()
        .execute_with(|| {
            assert_eq!(StakePallet::top_candidates().into_iter().map(|s| s.owner).collect::<Vec<u64>>(), vec![1, 2]);
            assert_noop!(StakePallet::init_leave_candidates(Origin::signed(11)), Error::<Test>::CandidateNotFound);
            assert_noop!(
            ...
            assert_noop!(StakePallet::candidate_stake_less(Origin::signed(1), 1), Error::<Test>::CannotStakeIfLeaving);
            assert_noop!(StakePallet::candidate_stake_more(Origin::signed(1), 1), Error::<Test>::CannotStakeIfLeaving);
            assert_noop!(StakePallet::init_leave_candidates(Origin::signed(1)), Error::<Test>::AlreadyLeaving);
            ...
            roll_to(5, vec![]);
            assert_noop!(StakePallet::execute_leave_candidates(Origin::signed(2), 2), Error::<Test>::NotLeaving);
            assert_noop!(StakePallet::execute_leave_candidates(Origin::signed(2), 1), Error::<Test>::CannotLeaveYet);
            ...
            assert_eq!(
                state.total,
                StakePallet::top_candidates().into_bounded_vec().iter().find(|other| other.owner == 1).unwrap().amount
            );
            assert_eq!(StakePallet::selected_candidates().into_inner(), vec![1, 2]);
...
fn adjust_reward_rates() {
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            // put some rewards per block
            <crate::RewardPerBlock<Test>>::put(100 * DECIMALS);

            // reward once in first year
            roll_to(2, authors.clone());
            assert_eq!(Balances::free_balance(&1), 10_000_000 * DECIMALS + (20 * DECIMALS));
            assert_eq!(Balances::free_balance(&2), 90_000_000 * DECIMALS + (80 * DECIMALS));
            ...
            let c_rewards_1 =
                Balances::free_balance(&1).saturating_sub(10_000_000 * DECIMALS).saturating_sub(c_rewards_0);
            let d_rewards_1 =
                Balances::free_balance(&2).saturating_sub(90_000_000 * DECIMALS).saturating_sub(d_rewards_0);
            assert!(
                c_rewards_0 == c_rewards_1,
                "Reward rate must be same every year.left {:?}, right {:?}",
                c_rewards_0,
                c_rewards_1
            );
            ...
            assert!(d_rewards_1 == d_rewards_2);
        });
}
...
fn increase_max_candidate_stake() {
    ...
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            assert_noop!(StakePallet::candidate_stake_more(Origin::signed(1), 1), Error::<Test>::ValStakeAboveMax);

            assert_ok!(StakePallet::set_max_candidate_stake(Origin::root(), max_stake + 1));
            assert_eq!(last_event(), MetaEvent::StakePallet(Event::MaxCandidateStakeChanged(max_stake + 1)));
...
fn decrease_max_candidate_stake() {
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            assert_noop!(StakePallet::candidate_stake_more(Origin::signed(1), 0), Error::<Test>::ValStakeZero);
            assert_noop!(StakePallet::candidate_stake_less(Origin::signed(1), 0), Error::<Test>::ValStakeZero);
            assert_noop!(StakePallet::candidate_stake_more(Origin::signed(1), 1), Error::<Test>::ValStakeAboveMax);
            assert_ok!(StakePallet::candidate_stake_less(Origin::signed(1), 50));
            assert_noop!(StakePallet::set_max_candidate_stake(Origin::root(), 9), Error::<Test>::CannotSetBelowMin);
        });
}
...
fn force_remove_candidate() {
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            assert!(!StakePallet::top_candidates().contains(&StakeOf::<Test> {
                owner: 1,
                amount: 100
            }));
            ...
            assert_noop!(StakePallet::force_remove_candidate(Origin::root(), 4), Error::<Test>::CandidateNotFound);
...
fn prioritize_delegators() {
    ExtBuilder::default()
        .with_balances(vec![(1, 1000), (2, 1000), (3, 1000), (4, 1000), (5, 1000), (6, 1000), (7, 1000)])
        .with_collators(vec![(1, 100), (2, 100), (3, 100)])
...
fn authorities_per_round() {
    ...
    ExtBuilder::default()
        ...
        .build()
        .execute_with(|| {
            let reward = 50 * DECIMALS;
            <crate::RewardPerBlock<Test>>::put(reward);
            ...
            roll_to(4, authors.clone());
            assert_eq!(Balances::free_balance(1), stake + reward);
            ...
            assert_eq!(Balances::free_balance(1), stake + reward * 2);

            // roll to last block of round 2
            roll_to(14, authors.clone());
            assert_eq!(Balances::free_balance(1), stake + reward * 3);

            // roll to last block of round 3
            roll_to(19, authors);
            assert_eq!(Balances::free_balance(1), stake + reward * 4);
        });
}
...
fn replace_lowest_delegator() {
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            assert!(StakePallet::delegator_state(5).is_none());
            assert_eq!(
                StakePallet::candidate_pool(1).unwrap().delegators.into_bounded_vec().into_inner(),
                vec![
                    Stake {
                        owner: 2,
                        amount: 51
...
fn network_reward_multiple_blocks() {
    ...
    ExtBuilder::default().with_balances(collators.clone()).with_collators(collators).build().execute_with(|| {
        ...
        assert_eq!(total_issuance + network_reward, <Test as Config>::Currency::total_issuance());
        ...
        assert_eq!(total_issuance + 2 * network_reward, <Test as Config>::Currency::total_issuance());
        ...
        assert_eq!(total_issuance + 100 * network_reward, <Test as Config>::Currency::total_issuance());
        ...
        assert_eq!(total_issuance + 101 * network_reward, <Test as Config>::Currency::total_issuance());
    });
}
