...
impl<T: Ord + Clone, S: Get<u32>> OrderedSet<T, S> {
    ...
    /// * Err(true) if the set is full and the new element has a lower rank than the lowest element in the set.
...
mod tests {
    ...
    fn try_insert_replace_stake() {
        ...
        assert_eq!(
            set.try_insert_replace(StakeOf::<Test> {
                owner: 1,
                amount: 0
            }),
            Err(false)
    ...
    fn linear_search() {
        ...
        assert_eq!(
            set.linear_search(&StakeOf::<Test> {
                owner: 1,
                amount: 0
            }),
            Ok(0)
