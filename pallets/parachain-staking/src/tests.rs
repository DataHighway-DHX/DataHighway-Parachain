...
use sp_runtime::{traits::Zero, Permill, Perquintill, SaturatedConversion};

use crate::{
	mock::{
		events, last_event, roll_to, AccountId, Balance, Balances, BlockNumber,
	...
	CandidatePool, Config, Error, Event, Event as StakeEvent, STAKING_ID,
};
...
fn join_collator_candidates() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				last_event(),
				StakeEvent::JoinedCollatorCandidates(7, 10u128)
			);
...
fn collator_selection_chooses_top_candidates() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				last_event(),
				StakeEvent::JoinedCollatorCandidates(6, 69u128)
			);
...
fn round_transitions() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				last_event(),
				StakeEvent::BlocksPerRoundSet(1, 5, 5, 3)
			);
...
fn increase_max_candidate_stake() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				last_event(),
				StakeEvent::MaxCandidateStakeChanged(max_stake + 1)
			);
...
fn decrease_max_candidate_stake() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				last_event(),
				StakeEvent::MaxCandidateStakeChanged(50)
			);
