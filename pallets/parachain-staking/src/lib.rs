...
//! - Joining the set of collator candidates of which the best `MaxSelectedCandidates` are chosen to become active
//!   collators for the next session. That makes the set of active collators the set of block authors by handing it over
//!   to the session and the authority pallet.
...
//! ### Terminology
//!
//! - **Candidate:** A user which locks up tokens to be included into the set of authorities which author blocks and
//!   receive rewards for doing so.
//!
//! - **Collator:** A candidate that was chosen to collate this round.
//!
//! - **Delegator:** A user which locks up tokens for collators they trust. When their collator authors a block, the
//!   corresponding delegators also receive rewards.
//!
...
//!
//! - **Round (= Session):** A fixed number of blocks in which the set of collators does not change. We set the length
//!   of a session to the length of a staking round, thus both words are interchangeable in the context of this pallet.
//!
...
//! - `set_max_selected_candidates` - Change the number of collator candidates which can be selected to be in the set of
//!   block authors. Requires sudo.
//! - `set_blocks_per_round` - Change the number of blocks of a round. Shorter rounds enable more frequent changes of
//!   the selected candidates, earlier unlockal from unstaking and earlier collator leaving. Requires sudo.
//! - `increase_max_candidate_stake_by` - Increase the maximum amount which can be staked by a collator candidate.
//! - `decrease_max_candidate_stake_by` - Decrease the maximum amount which can be staked by a collator candidate.
//! - `join_candidates` - Join the set of collator candidates by staking at least `MinCandidateStake` and at most
//!   `MaxCollatorCandidateStake`.
//! - `init_leave_candidates` - Request to leave the set of collators. Unstaking and storage clean-up is delayed until
//!   executing the exit at least ExitQueueDelay rounds later.
//! - `candidate_stake_more` - Increase your own stake as a collator candidate by the provided amount up to
//!   `MaxCollatorCandidateStake`.
//! - `candidate_stake_less` - Decrease your own stake as a collator candidate by the provided amount down to
//!   `MinCandidateStake`.
...
//! - `leave_delegators` - Leave the set of delegators and revoke all delegations. Since delegators do not have to run a
//!   node and cannot be selected to become block authors, this exit is not delayed like it is for collator candidates.
//! - `revoke_delegation` - Revoke a single delegation to a collator candidate.
//! - `delegator_stake_more` - Increase your own stake as a delegator and the delegated collator candidate's total
//!   stake.
//! - `delegator_stake_less` - Decrease your own stake as a delegator and the delegated collator candidate's total stake
//!   by the provided amount down to `MinDelegatorStake`.
//! - `unlock_unstaked` - Attempt to unlock previously unstaked balance from any account. Succeeds if at least one
//!   unstake call happened at least `StakeDuration` blocks ago.
//!
//! ## Genesis config
//!
...
//! ## Assumptions+
//!
//! - At the start of session s(i), the set of session ids for session s(i+1) are chosen. These equal the set of
//!   selected candidates. Thus, we cannot allow collators to leave at least until the start of session s(i+2).
...
pub mod pallet {
    ...
    pub type GenesisStaker<T> =
        Vec<(<T as frame_system::Config>::AccountId, Option<<T as frame_system::Config>::AccountId>, BalanceOf<T>)>;
    ...
    impl<T: Config> GenesisBuild<T> for GenesisConfig<T> {
        fn build(&self) {
            assert!(self.reward_per_block > Zero::zero(), "reward_per_block should be non zero");
            ...
            for &(ref actor, ref opt_val, balance) in &self.stakers {
                assert!(T::Currency::free_balance(actor) >= balance, "Account does not have enough balance to stake.");
                if let Some(delegated_val) = opt_val {
                    ...
                    assert_ok!(<Pallet<T>>::join_candidates(T::Origin::from(Some(actor.clone()).into()), balance));
                }
            }
    ...
    impl<T: Config> Pallet<T> {
        ...
        ///
        /// # <weight>
        /// - The transaction's complexity is mainly dependent on updating the `SelectedCandidates` storage in
        ///   `select_top_candidates` which in return depends on the number of `MaxSelectedCandidates` (N).
        ...
        pub fn set_blocks_per_round(origin: OriginFor<T>, new: T::BlockNumber) -> DispatchResult {
            ...
            Self::deposit_event(Event::BlocksPerRoundSet(old_round.current, old_round.first, old_round.length, new));
            Ok(())
        }
        ...
        /// - Reads: MaxCollatorCandidateStake, 2 * N * CandidatePool, TopCandidates, BlockNumber, D * DelegatorState, D
        ///   * Unstaking
        /// - Writes: MaxCollatorCandidateStake, N * CandidatePool, D * DelegatorState, (D + 1) * Unstaking
        /// - Kills: CandidatePool, DelegatorState for all delegators which only delegated to the candidate
        /// # </weight>
        ...
        pub fn force_remove_candidate(
            ...
            Ok(Some(<T as Config>::WeightInfo::force_remove_candidate(num_collators, num_delegators)).into())
        }
        ...
        /// candidate bounded by `MaxDelegatorsPerCollator`.
        /// - Reads: [Origin Account], DelegatorState, MaxCollatorCandidateStake, Locks, TotalCollatorStake,
        ...
        pub fn join_candidates(origin: OriginFor<T>, stake: BalanceOf<T>) -> DispatchResultWithPostInfo {
            ...
            Ok(Some(<T as pallet::Config>::WeightInfo::join_candidates(n, T::MaxDelegatorsPerCollator::get())).into())
        }
        ...
        pub fn init_leave_candidates(origin: OriginFor<T>) -> DispatchResultWithPostInfo {
            ...
            Ok(Some(<T as pallet::Config>::WeightInfo::init_leave_candidates(num_collators, num_delegators)).into())
        }
        ...
        pub fn cancel_leave_candidates(origin: OriginFor<T>) -> DispatchResultWithPostInfo {
            ...
            Ok(Some(<T as pallet::Config>::WeightInfo::cancel_leave_candidates(n, T::MaxDelegatorsPerCollator::get()))
                .into())
        }
        ...
        /// this candidate bounded by `MaxDelegatorsPerCollator` and U is the
        /// number of locked unstaking requests bounded by `MaxUnstakeRequests`.
        /// - Reads: [Origin Account], Locks, TotalCollatorStake, MaxCollatorCandidateStake, TopCandidates,
        ///   CandidatePool
        /// - Writes: Locks, TotalCollatorStake, CandidatePool, TopCandidates
        /// # </weight>
        ...
        pub fn candidate_stake_more(origin: OriginFor<T>, more: BalanceOf<T>) -> DispatchResultWithPostInfo {
            ...
            ensure!(state.stake <= MaxCollatorCandidateStake::<T>::get(), Error::<T>::ValStakeAboveMax);
        ...
        /// `MaxTopCandidates` and D is the number of delegators for this
        /// candidate bounded by `MaxDelegatorsPerCollator`.
        /// - Reads: [Origin Account], Unstaking, TopCandidates, MaxSelectedCandidates, CandidatePool
        ...
        pub fn candidate_stake_less(origin: OriginFor<T>, less: BalanceOf<T>) -> DispatchResultWithPostInfo {
            ...
            Ok(Some(<T as pallet::Config>::WeightInfo::candidate_stake_less(n, T::MaxDelegatorsPerCollator::get()))
                .into())
        }
        ...
        /// `MaxTopCandidates` and D is the number of delegators for this
        /// candidate bounded by `MaxDelegatorsPerCollator`.
        /// - Reads: [Origin Account], DelegatorState, TopCandidates, MaxSelectedCandidates, CandidatePool,
        ///   LastDelegation, Round
        ...
        pub fn join_delegators(
            ...
            Ok(Some(<T as pallet::Config>::WeightInfo::join_delegators(n, T::MaxDelegatorsPerCollator::get())).into())
        }
        ...
        pub fn delegate_another_candidate(
            ...
            ensure!(state.delegators.try_insert(delegation.clone()).unwrap_or(true), Error::<T>::DelegatorExists);
        ...
        /// - Reads: [Origin Account], DelegatorState, BlockNumber, Unstaking, TopCandidates, MaxSelectedCandidates, C *
        ///   CandidatePool,
        /// - Writes: Unstaking, CandidatePool, TotalCollatorStake,
        ...
        /// Weight: O(C) where C is the number of delegations for this delegator
        /// which is bounded by by `MaxCollatorsPerDelegator`.
        /// - Reads: [Origin Account], DelegatorState, BlockNumber, Unstaking, Locks, TopCandidates, CandidatePool,
        ...
        pub fn delegator_stake_more(
            ...
            let delegator_total =
                delegations.inc_delegation(candidate.clone(), more).ok_or(Error::<T>::DelegationNotFound)?;
        ...
        /// # <weight>
        /// Weight: O(1)
        /// - Reads: [Origin Account], DelegatorState, BlockNumber, Unstaking, TopCandidates, CandidatePool,
        ///   MaxSelectedCandidates
        ...
        pub fn delegator_stake_less(
            ...
            Ok(Some(<T as pallet::Config>::WeightInfo::delegator_stake_less(n, T::MaxDelegatorsPerCollator::get()))
                .into())
        }
    ...
    impl<T: Config> Pallet<T> {
        ...
        fn update_top_candidates(
            ...
            if let Some((maybe_old_idx, top_candidates)) = maybe_top_candidate_update {
                ...
                let is_collating =
                    top_candidates.linear_search(&new_stake).map(|i| i < max_selected_candidates).unwrap_or(false);
        ...
        fn update_total_stake_by(
            ...
            TotalCollatorStake::<T>::mutate(|total| {
                total.collators = total.collators.saturating_sub(sub_collators).saturating_add(add_collators);
                total.delegators = total.delegators.saturating_sub(sub_delegators).saturating_add(add_delegators);
            });
        }
        ...
        /// this collator bounded by `MaxCollatorsPerDelegator`.
        /// - Reads: [Origin Account], DelegatorState, BlockNumber, Unstaking, Locks, TopCandidates, D * CandidatePool,
        ///   MaxSelectedCandidates
        /// - Writes: Unstaking, Locks, DelegatorState, CandidatePool, TotalCollatorStake
        /// - Kills: DelegatorState if the delegator has not delegated to another collator
        /// # </weight>
        fn delegator_revokes_collator(acc: T::AccountId, collator: T::AccountId) -> Result<u32, DispatchError> {
            ...
            let remaining = delegator.rm_delegation(&collator).ok_or(Error::<T>::DelegationNotFound)?;
        ...
        fn delegator_leaves_collator(delegator: T::AccountId, collator: T::AccountId) -> DispatchResult {
            ...
            Self::deposit_event(Event::DelegatorLeftCollator(delegator, collator, delegator_stake, new_total));
            Ok(())
        }
        ...
        /// * Or `None`, signalling the delegator state should be cleared once the transaction cannot fail anymore.
        ...
        ) -> Result<(CandidateOf<T, T::MaxDelegatorsPerCollator>, Option<ReplacedDelegator<T>>), DispatchError>
        {
            // attempt to replace the last element of the set
            let stake_to_remove = state.delegators.try_insert_replace(stake.clone()).map_err(|err_too_many| {
                if err_too_many {
        ...
        fn increase_lock(who: &T::AccountId, amount: BalanceOf<T>, more: BalanceOf<T>) -> Result<u32, DispatchError> {
            ...
            <Unstaking<T>>::try_mutate(who, |unstaking| -> DispatchResult {
                ...
                // locked
                let mut amt_consuming_unstaking = if more.is_zero() {
                    amount
                } else {
                    more
                };
                ...
                for (block_number, locked_balance) in unstaking.clone() {
                    if amt_consuming_unstaking.is_zero() {
                        ...
                        unstaking.try_insert(block_number, delta).map_err(|_| Error::<T>::NoMoreUnstaking)?;
        ...
        fn prep_unstake(who: &T::AccountId, amount: BalanceOf<T>, is_removal: bool) -> DispatchResult {
            ...
            ensure!(unstaking.len().saturated_into::<u32>() < allowed_unstakings, Error::<T>::NoMoreUnstaking,);
            ...
            unstaking.try_insert(unlock_block, amount).map_err(|_| Error::<T>::NoMoreUnstaking)?;
        ...
        fn do_unlock(who: &T::AccountId) -> Result<u32, DispatchError> {
            ...
            total_locked = if let Some(BalanceLock {
                amount,
                ..
        ...
        fn get_delegation_counter(delegator: &T::AccountId) -> Result<DelegationCounter, DispatchError> {
            ...
            ensure!(T::MaxDelegationsPerRound::get() > counter, Error::<T>::DelegationsPerRoundExceeded);
